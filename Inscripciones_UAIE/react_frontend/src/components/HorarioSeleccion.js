...
function HorarioSeleccion() {
  ...
  const validarTraslapes = () => {
    ...
    materiasSeleccionadas.forEach((materiaA, index) => {
      const horariosA = Object.values(materiaA.horarios); // Obtener horarios por días
      materiasSeleccionadas.slice(index + 1).forEach((materiaB) => {
        const horariosB = Object.values(materiaB.horarios);
  
        horariosA.forEach((horarioA, diaIndex) => {
          if (
            horarioA &&
            horariosB[diaIndex] &&
            hayTraslape(horarioA.split("-"), horariosB[diaIndex].split("-"))
          ) {
            conflictos.push({
